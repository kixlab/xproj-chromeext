...
let questions = function () {
  ...
  if (qs[qidx].type !== 'end') {
    ...
    if (qs[qidx].type === 'status') {
      ...
      str += '<button class="buttons progressButtons">이행 중</button> <button class="buttons progressButtons">이행 완료</button>'                  
    } else {
      str += '<button class="buttons yesButton">예</button> <button class="buttons noButton">아니오</button>'      
    }
  } else {
...
let addButtons = function () {
  $('#articleBodyContents').append('<div id="myContainer" title="공약 더 알아보기"><h3>기사와 관련있는 ' + officialName + '의 ' + category  + ' 분야 공약입니다. 클릭하시면 자세한 내용을 알아보실 수 있습니다.</h3></div>')
  $('#myContainer').dialog({
    position: {
      my: 'right center',
      at: 'right center+25%'
  ...
  qs = [
    ...
    // },
    {
      content: '응답해주셔서 감사합니다. 공약에 대해 ' + officialName +  '에게 하실 말씀이 있으시다면 남겨주세요.',
